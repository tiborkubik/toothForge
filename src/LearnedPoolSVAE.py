...
class LearnedPoolSVAE(torch.nn.Module):
    def __init__(self,
                 k: int,
                 size_latent: int = 16,
        ...
        super(LearnedPoolSVAE, self).__init__()
        self.k = k
        ...
        # Instead of one linear layer, we now need two: for mean and log variance
        self.encoder_mu = nn.Linear(encoder_linear[-2], encoder_linear[-1])
        self.encoder_logvar = nn.Linear(encoder_linear[-2], encoder_linear[-1])
